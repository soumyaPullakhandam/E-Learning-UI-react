...
const topicList = (props) => {
    ...
    if (!!props.coursedetails && props.coursedetails.hasOwnProperty('title')) {
        ...
        topics = props.topics.map(
            (topic,index) => {
                ...
                return (<Link to={ref} key={topic.id}
                              className="nav-link align-items-center d-flex list-group-item-secondary justify-content-between list-group-item list-group-item-action">
                    {topic.title}
                    <div className="btn-group btn-group-sm">
                        <button className="btn btn-outline-light" data-index={index}
        ...
        const newtopicbody = (<NewTopic title={props.mytopic.title}
                                        ...
                                        onLecSubmit={(evt) => props.onLecSubmit(evt)}
                                        handleNewTopicSubmit={(evt) => props.handleNewTopicSubmit(evt)}
                                        topicLecUpdate={(evt) => props.topicLecUpdate(evt)}
                                        topicLecDelete={(evt) => props.topicLecDelete(evt)}
                                        onLecUpdate={(evt) => props.onLecUpdate(evt)}
